            ...
            <div class="dr-mmeta">
                ...
                <div class="dr-mcards">
                    <div class="dr-mcard">
                        ...
                        {{-- Blue like the other two captions: the three chips are one
                             row and a lone grey line under the date read as disabled. --}}
                        <small class="dr-mcard__link">{{ $mMonth ?: 'Date' }}</small>
                    </div>
                    ...
                    <div class="dr-mcard">
                        <span class="dr-mcard__ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15 14"/></svg></span>
                        <strong>{{ $hasTime ? $mTime : 'TBA' }}</strong>
                        {{-- Blue when it carries a real end time (matching the other
                             chips), grey when it's only the "Doors Open" label — this
                             card isn't tappable, so blue has to mean content. --}}
                        <small @class(['dr-mcard__link' => $evEnd !== ''])>{{ $evEnd !== '' ? 'till ' . $evEnd : ($hasTime ? 'Doors Open' : 'Time') }}</small>
                    </div>
                    @endif
                </div>
            </div>
