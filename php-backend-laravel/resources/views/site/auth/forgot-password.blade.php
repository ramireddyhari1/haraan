...
<div class="authp">
    <div class="authp__card">
        ...
        <div class="authp__body">
            ...
            <p class="subtitle">Enter your email and we'll send you a link to set a new one.</p>

            @if(session('status'))<div class="auth-alert auth-alert--ok" role="status">{{ session('status') }}</div>@endif
            @if($errors->any())<div class="auth-alert" role="alert">{{ $errors->first() }}</div>@endif

            <form class="pw-form" method="POST" action="{{ route('site.password.email') }}">
                @csrf
                <div class="auth-field">
                    <label for="fpEmail">Email</label>
                    <input type="email" name="email" id="fpEmail" class="auth-input" placeholder="you@example.com" value="{{ old('email') }}" required autocomplete="email" autocapitalize="off" spellcheck="false" autofocus>
                </div>
                <button type="submit" class="btn btn--solid btn--full btn--large">Send reset link</button>
            </form>
